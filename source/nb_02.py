...
for start in (0, 60, 90, 180):
    a = butane_at(start)
    o = LBFGS(a, logfile=None)
    if start == 0:                       # record this one, to watch below
        o.attach(lambda a=a: film.append(a.copy()), interval=1)
    o.run(fmax=FMAX, steps=400)
    minima[start] = a
...
# %% [markdown]
# Two distinct minima appear, near 180° and near ±60°. Left, the structure
# reached from 180°; right, the one reached from 60°. Look along the central
# C–C bond to see the difference.

# %%
show([minima[180], minima[60]])

# %% [markdown]
# The optimisation that started at 0°, with the two methyl groups eclipsed,
# played as a film. Each frame is one optimiser step: the methyls rotate
# apart until the forces vanish, and then the film stops.

# %%
show(film, animate=True, loop="forward")
