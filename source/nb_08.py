...
# %% [markdown]
# The six molecules in their box. The unit cell is drawn; the calculation
# sees its periodic images in every direction.
...
buffer = io.StringIO()
write(buffer, cell, format="proteindatabank")   # PDB carries the cell (CRYST1)
v = py3Dmol.view(width=420, height=340)
v.addModel(buffer.getvalue(), "pdb")
v.setStyle({"stick": {"radius": 0.14}, "sphere": {"scale": 0.24}})
v.addUnitCell(); v.zoomTo(); v.show()
