...
def reactivity(atoms):
    ...
    def evaluate(charge, mult):
        ...
        a.calc = AIMNet2ASE(AIMNet2Calculator(MODEL), charge=charge, mult=mult)
...
molecule = build(entry["smiles"])
molecule.calc = AIMNet2ASE(AIMNet2Calculator(MODEL), charge=0, mult=1)
